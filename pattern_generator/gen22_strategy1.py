...
parser = argparse.ArgumentParser(description='gen22')
...
W_ROW = 32#must be the multiple of 16, most 32
DIM = 256*4
A_COL = 124#must be the multiple of 1, 4 for maxpooling, most 128
...
for k in range(W_ROW):
  ...
  for t in range(int(DIM)/4):
    W_file = W_file + str(twos_complement(W[k][t], 2))
  f.write(W_file+'\n')
...
for k in range(0, A_COL):
  ...
  for t in range(0, 256):
    I_file = I_file + str(twos_complement(A[t][k], 2))

...
parser = argparse.ArgumentParser(description='gen88')
...
DIM = 16
A_COL = 104*4#must be the multiple of 16, most 128*4
...
for k in range(0, W_ROW, 4):
  W_file = ''
  for t in range(DIM):
    W_file = W_file + str(twos_complement(W[k+3][t], 8))
  for t in range(DIM):
    W_file = W_file + str(twos_complement(W[k+2][t], 8))
  for t in range(DIM):
    W_file = W_file + str(twos_complement(W[k+1][t], 8))
  for t in range(DIM):
    W_file = W_file + str(twos_complement(W[k+0][t], 8))
  f.write(W_file+'\n')
...
for k in range(0, int(A_COL/4), 4):
  I_file = ''
  for t in range(0, 16):
    I_file = I_file + str(twos_complement(A[t][k+3], 8))
  for t in range(0, 16):
    I_file = I_file + str(twos_complement(A[t][k+2], 8))
  for t in range(0, 16):
    I_file = I_file + str(twos_complement(A[t][k+1], 8))
  for t in range(0, 16):
    I_file = I_file + str(twos_complement(A[t][k+0], 8))
  
  f.write(I_file+'\n')

f = open('./activation/activation_1.txt', 'w')
for k in range(int(A_COL/4), int(A_COL/4)*2, 4):
  I_file = ''
  for t in range(0, 16):
    I_file = I_file + str(twos_complement(A[t][k+3], 8))
  for t in range(0, 16):
...
for k in range(int(A_COL/4)*2, int(A_COL/4)*3, 4):
  I_file = ''
  for t in range(0, 16):
    I_file = I_file + str(twos_complement(A[t][k+3], 8))
  for t in range(0, 16):
...
for k in range(int(A_COL/4)*3, A_COL, 4):
  I_file = ''
  for t in range(0, 16):
    I_file = I_file + str(twos_complement(A[t][k+3], 8))
  for t in range(0, 16):

...
parser = argparse.ArgumentParser(description='gen84')
...
DIM = 32
A_COL = 100*4#must be the multiple of 8, 16 for maxpooling, most 128*4
...
for k in range(0, W_ROW, 4):
  W_file = ''
  for t in range(DIM):
    W_file = W_file + str(twos_complement(W[k+3][t], 4))
  for t in range(DIM):
    W_file = W_file + str(twos_complement(W[k+2][t], 4))
  for t in range(DIM):
...
for k in range(0, int(A_COL/4), 2):
  I_file = ''
  for t in range(0, 32):
...
for k in range(int(A_COL/4), int(A_COL/4)*2, 2):
  I_file = ''
  for t in range(0, 32):
    I_file = I_file + str(twos_complement(A[t][k+1], 8))
  for t in range(0, 32):
...
for k in range(int(A_COL/4)*2, int(A_COL/4)*3, 2):
  I_file = ''
  for t in range(0, 32):
    I_file = I_file + str(twos_complement(A[t][k+1], 8))
  for t in range(0, 32):
...
for k in range(int(A_COL/4)*3, A_COL, 2):
  I_file = ''
  for t in range(0, 32):
    I_file = I_file + str(twos_complement(A[t][k+1], 8))
  for t in range(0, 32):
